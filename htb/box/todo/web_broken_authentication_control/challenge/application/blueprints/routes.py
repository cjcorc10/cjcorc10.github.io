## This file is used to define how to handle the URL routes requested by users.
from flask import Blueprint, request, jsonify, session, render_template, g
# this is where verify_integrity is loaded from
from application.util import verify_integrity
from application.models import todo

# 2 Blueprint instances are created. Blue objects are used to organize related routes and view into separate groups.
main  = Blueprint('main', __name__)
api   = Blueprint('api', __name__)

#decorator of main to handle the '/' URL.
@main.route('/')
def index():
	context = {
		'list_access': g.user,
...
# this is used to add items to the list
